...
function makeResult(findings: Finding[], over: Partial<ScanResult> = {}): ScanResult {
  return {
    root: '/repo',
    score: 100,
    files: ['AGENTS.md'],
...
describe('renderComment', () => {
  it('renders a clean "No findings" comment with the hidden marker', () => {
    ...
    expect(body).toContain('## agents-md-xray report');
    ...
    expect(body).not.toContain('| Severity | Rule | File | Evidence | Remediation |');
  });

  it('renders a findings table with mapped severities and the rule id', () => {
    const body = renderComment(makeResult([FAIL_FINDING, WARN_FINDING], { score: 75 }));
    expect(body).toContain('- **Score:** 75/100');
    expect(body).toContain('- **Findings:** 2');
    expect(body).toContain('| Severity | Rule | File | Evidence | Remediation |');
    expect(body).toContain('| --- | --- | --- | --- | --- |');
    expect(body).toContain('| error | dangerous-instruction.system-override | AGENTS.md | ignore previous instructions |');
    expect(body).toContain('| warning | context-bloat.too-long | AGENTS.md |  |');
  });
...
describe('evaluateThreshold', () => {
  const fail: Finding[] = [{ id: 'a', severity: 'fail', title: 't', message: 'm' }];
  const warn: Finding[] = [{ id: 'b', severity: 'warn', title: 't', message: 'm' }];
  const info: Finding[] = [{ id: 'c', severity: 'info', title: 't', message: 'm' }];
  ...
  it('throws a clear error on an invalid value', () => {
    expect(() => evaluateThreshold(fail, 'sometimes')).toThrow(/Invalid fail-on value/);
...
describe('summarize', () => {
  it('extracts score and findings count', () => {
    expect(summarize(makeResult([FAIL_FINDING, WARN_FINDING], { score: 64 }))).toEqual({
      score: 64,
      findingsCount: 2,
    });
  });
...
describe('runtime mirror parity (action/lib.mjs)', () => {
  ...
  const cases: ScanResult[] = [
    makeResult([]),
    makeResult([FAIL_FINDING, WARN_FINDING], { score: 75, files: ['AGENTS.md', 'docs/CLAUDE.md'] }),
    makeResult(
      [
        {
          id: 'r|x',
  ...
  it('renderComment produces byte-identical output', () => {
    for (const result of cases) {
      ...
      expect(runtime.renderComment(result, { detailsUrl: 'https://example.com/run/7', maxFindings: 3 })).toBe(
        renderComment(result, { detailsUrl: 'https://example.com/run/7', maxFindings: 3 }),
      );
    }
  });
  ...
  it('evaluateThreshold matches for valid values', () => {
    ...
    for (const value of ['error', 'warning', 'off', 'ERROR', ' warning ']) {
      expect(runtime.evaluateThreshold(findings, value)).toEqual(evaluateThreshold(findings, value));
    }
  });

  it('evaluateThreshold both throw on invalid values', () => {
    expect(() => runtime.evaluateThreshold([], 'nope')).toThrow(/Invalid fail-on value/);
    expect(() => evaluateThreshold([], 'nope')).toThrow(/Invalid fail-on value/);
  });
