...
export const PR_COMMENT_MARKER = "<!-- rootmark-pr-comment -->";

/** Maximum number of findings rendered in the PR comment table. */
export const MAX_FINDINGS_IN_COMMENT = 25;

const COMMENT_FOOTER =
  "<sub>Top-level summary from Rootmark. These are file-level findings; " +
...
export function evaluateThreshold(findings, failOnRaw) {
  const value = String(failOnRaw ?? "")
    .trim()
    .toLowerCase();
  if (value !== "off" && value !== "warning" && value !== "error") {
    throw new Error(
      `Invalid fail-on value: "${failOnRaw}". Supported values: error, warning, off.`,
    );
  }
  if (value === "off") {
  ...
  const threshold =
    value === "warning" ? SEVERITY_RANK.warn : SEVERITY_RANK.fail;
  const list = Array.isArray(findings) ? findings : [];
  const shouldFail = list.some(
